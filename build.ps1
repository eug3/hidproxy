...
try {
    ...
    cmake --build . --config $Config --target Xsjzb_hook --target Xsjzb_launcher
    ...
    Copy-Item "injector\build\bin\$Config\Xsjzb_hook.dll" -Destination "$buildBinDir\" -Force
    Copy-Item "injector\build\bin\$Config\Xsjzb_launcher.exe" -Destination "$buildBinDir\" -Force
    
    Write-Host ""
    Write-Host "Build succeeded!" -ForegroundColor Green
    Write-Host ""
    Write-Host "Output files:" -ForegroundColor Cyan
    Write-Host "  - DLL:      $buildBinDir\Xsjzb_hook.dll" -ForegroundColor Yellow
    Write-Host "  - Launcher: $buildBinDir\Xsjzb_launcher.exe" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "To deploy:" -ForegroundColor Cyan
    Write-Host "  Copy-Item '$buildBinDir\Xsjzb_hook.dll' -Destination 'C:\Xsj_Soft\Xsjzb\' -Force" -ForegroundColor Gray
